...
ローカルネットワーク: 設定ページで "ローカルネットワーク接続を許可" を有効にします. 設定ページに端末の現在のアドレス (Wi-Fi の変化に追従) と, クライアントが同じネットワークに参加する必要があることが表示されます. ゲストネットワーク, AP 分離, PC のファイアウォールがよくある障害です. ローカルネットワークからのペアリング要求は明示され, サーバーがネットワークから到達可能な間は毎日通知で知らせます. 通知はオフにできます. Android 17 以降では, このプラグインの付近のデバイス権限を許可するとローカルネットワークのデバイスに接続できます. AutoJs6 の許可はプラグインに引き継がれません. インターネットとループバック接続にはこの権限は不要です. この操作でローカルネットワークを使用する場合は, AutoJs6 のプラグインセンターからこのプラグインを開き, 付近のデバイスを許可してください. AutoJs6 の許可は引き継がれません.
...
- Android 17 以降では, このプラグインの付近のデバイス権限を許可するとローカルネットワークのデバイスに接続できます. AutoJs6 の許可はプラグインに引き継がれません. インターネットとループバック接続にはこの権限は不要です.
