...
로컬 네트워크: 설정 페이지에서 "로컬 네트워크 연결 허용" 을 켭니다. 그러면 설정 페이지에 휴대전화의 현재 주소 (Wi-Fi 변경을 따라감) 와 클라이언트가 같은 네트워크에 있어야 한다는 안내가 표시됩니다. 게스트 네트워크, AP 격리, PC 방화벽이 흔한 장애 요인입니다. 로컬 네트워크에서 온 페어링 요청은 표시되며, 서버가 네트워크에서 접근 가능한 동안 매일 알림으로 상기시킵니다. 알림은 끌 수 있습니다. Android 17 이상에서는 AutoJs6 플러그인 센터에서 이 플러그인을 켜기 전에 주변 기기 권한을 허용하세요. 이 플러그인의 설정 페이지에서도 로컬 네트워크 권한을 관리할 수 있습니다. 권한이 없으면 플러그인이 꺼진 상태로 유지되고 자동 시작은 알림 없이 건너뜁니다. 이 권한은 플러그인에 속하며 AutoJs6 권한과 별개입니다.
...
- Binder 진입점과 설정 페이지는 `org.autojs.permission.PLUGIN` 서명 권한으로 보호되므로 AutoJs6 만 접근할 수 있습니다. 페어링 대화상자, 그 리시버, 릴리스 기록 페이지는 내보내지 않습니다. 리스너를 담은 포그라운드 서비스만 adb (`android.permission.DUMP`) 를 받아들이며, 이것이 개발자의 시작 / 중지 스위치입니다.
- INTERNET 권한은 플러그인 자체 HTTP 리스너에만 사용됩니다. 플러그인은 외부 요청을 보내지 않으며 데이터를 수집하지 않습니다. 평문 HTTP 는 네트워크 보안 구성에 따라 루프백 주소로만 허용됩니다.
- 서버는 기본적으로 127.0.0.1 에서만 수신합니다. 로컬 네트워크 접근은 켜기 전까지 꺼져 있으며, 켠 뒤에도 토큰, 페어링 확인, Host 허용 목록, 속도 제한이 로컬 네트워크에서 그대로 적용되고, 켜져 있는 동안 매일 알림으로 알려 줍니다.
- Android 17 이상에서는 AutoJs6 플러그인 센터에서 이 플러그인을 켜기 전에 주변 기기 권한을 허용하세요. 이 플러그인의 설정 페이지에서도 로컬 네트워크 권한을 관리할 수 있습니다. 권한이 없으면 플러그인이 꺼진 상태로 유지되고 자동 시작은 알림 없이 건너뜁니다. 이 권한은 플러그인에 속하며 AutoJs6 권한과 별개입니다.
- 액세스 토큰은 안전한 난수원에서 생성되어 Android Keystore 의 AES-GCM 키로 감싼 뒤 플러그인의 비공개이며 백업되지 않는 저장소에 보관됩니다. 백업과 기기 간 전송은 비활성화되어 있습니다. 설정 페이지는 마지막 4 자만 표시하고, 전체 토큰 대화상자는 스크린샷을 차단하며, 복사본은 클립보드에서 민감 정보로 표시됩니다.
- 로그에는 토큰, 요청 본문, 파일 내용, 스크린샷이 절대 포함되지 않습니다. 플러그인은 도구 이름, 클라이언트 이름, 토큰 지문만 기록합니다. 실제 파일과 스크린샷 호출 중에 두 기기에서 logcat 으로 검증했습니다 (docs/dev/p6-security-audit.md).
- 도구 호출은 AutoJs6 기능 브로커를 거치며 호스트 자체에 허용된 범위를 절대 넘지 않습니다. 셸 명령, 파일 삭제, 제스처는 해당 그룹을 켜기 전까지 꺼져 있고, root 셸은 추가로 전용 스위치와 호스트 승인이 필요합니다.
- 페어링은 설정 페이지에서 하나씩 또는 한꺼번에 취소할 수 있습니다. 취소된 클라이언트는 다음 도구 호출 전에 휴대폰에서 다시 확인해야 합니다. 토큰을 교체해도 페어링은 유지되지만 이전 토큰을 계속 쓰는 클라이언트는 차단됩니다.
...
- `수정` 설정 버튼, 스위치 트랙과 손잡이, 입력란, 단일 선택 및 대화상자 버튼에 호스트 테마 색상을 적용하고 밝은 모드와 어두운 모드의 대비 개선
