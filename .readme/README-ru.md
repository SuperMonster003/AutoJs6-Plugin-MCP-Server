...
Оба пути используют один и тот же токен и одно и то же сопряжение на телефоне. Мост stdio для клиентов без поддержки HTTP планируется отдельно.
...
Claude Desktop запускает только stdio-серверы; мост для него запланирован (см. дорожную карту).
