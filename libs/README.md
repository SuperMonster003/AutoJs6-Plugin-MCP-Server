...
- `mcp-server-api.aar` (host module `plugin-api/mcp-server-api`, the Binder contract of roadmap P1.1; staged in P2.3)
...
Both artifacts are release builds of the host modules; `mcp-server-api.aar` depends on
`common-plugin-api.aar` (the `PluginInfo` parcelable), so the two must come from the same host
contract line. When the host contract changes, restage both and extend the lock file in the same commit.
