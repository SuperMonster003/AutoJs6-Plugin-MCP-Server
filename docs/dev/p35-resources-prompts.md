...
Claude Code 2.1.257 was found, but both existing CLI configurations report loggedIn=false / authMethod=none. The maintainer was asked to log in or identify an authenticated configuration. Claude Code reading a sample and generating a runnable script remains unverified; Python HTTP evidence does not substitute for that acceptance item. The optional Offline-Docs provider and OEM activation matrix are not part of the completed evidence.
