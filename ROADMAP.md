...
- [ ] (发布) GitHub Release (tag `v1.0.0`, 签名 APK, changelog 文本); `AutoJs6-Official-Plugins-Index` 经 `tools/generate_official_plugin_index.py` 重新生成 (不手改 JSON), 追加 `release-manifests/<packageName>/<versionCode>.json` (真实 sha256 / size / signer); 宿主插件中心可发现并安装. (SOURCE, 2026-09-16, 未发布: 发布材料已就绪. 签名 APK `releases/autojs6-plugin-mcp-server-v1.0.1-e6f720e3.apk` (versionCode 54, 见上条); changelog 文本取 `assets/doc/CHANGELOG-en.md` / `CHANGELOG-zh-Hans.md` 的 v1.0.1 与 v1.0.0 节; 准入清单草案由忽略目录的 `build/tools/p7_release_manifest.py` (aapt2 badging + apksigner 证书 + sha256 / size) 生成到 `build/p7/54.json`: schemaVersion 1, owner SuperMonster003, repository AutoJs6-Plugin-MCP-Server, releaseTag v1.0.1, sourceCommit b0fb7278ba1da2244031688e1f0169b3d2c52a9f, packageName io.github.supermonster003.autojs6.plugin.mcp.server, versionName 1.0.1, versionCode 54, signerSha256 31a681fc...c213 (1 个), artifacts 1 项 (sha256 743419f4...1324, 1377446 字节); 不含 runtimeComponent / supportedAbis, 因插件未声明二者. `p7_manifest_dryrun.py` 以清单自身的 name / size / sha256 构造 GitHub 资产喂给索引仓库的 `bind_release_artifacts()`, 键集合 / 身份字段 / 证据格式通过; 索引仓库 `tools` 离线单元测试 28 / 28, 索引中 MCP 条目 0 个, GitHub 仓库尚未创建. 待维护者执行 (本会话不创建仓库, 不发布, 不推送, 不打 tag): (1) 在打 tag 的提交上重跑 `:app:appendDigestToReleasedFiles`, 以该产物与 `--commit <tag 提交>` 重生成清单; (2) 创建 GitHub 仓库与 Release tag `v1.0.1` (当前 VERSION_NAME, 非本条原文的 1.0.0), 上传该 APK, 资产名须与清单 artifacts.name 完全一致, 并把 `.changelog` 的 released_date 改为实际日期后重新生成; (3) 把清单复制到索引仓库 `release-manifests/io.github.supermonster003.autojs6.plugin.mcp.server/<versionCode>.json`, 运行 `python tools/generate_official_plugin_index.py` (不手改 JSON) 与 `python -m unittest discover -s tools -p 'test_*.py' -v`, 提交索引; (4) 在两台设备上从插件中心发现并安装, 完成 P2 验收端到端. 索引生成器以 GitHub Release 事实校验 sha256 / size 并从 tag 读取元数据, 草案不可绕过, 故本条保持未勾选.)
