...
- [x] (插件, 可选) `autojs6://docs/<路径>`: 安装了 `AutoJs6-Plugin-Offline-Docs` 时经宿主转读其资产, 未安装时不列出 (D20). (SOURCE: 宿主分支 `mcp-p35-docs` 提交 04c9c16b7 (worktree `AutoJs6-mcp-p35-docs`, 基于 master aa35b47aa, 未合并) 新增只读 `app.listDocs` / `app.readDoc`: `NodeBridgeDocsCatalog` 以 `OfflineDocsPluginStateMonitor` 信任的插件已校验清单为源, 目录由清单推导, 列表上限 2000 / 单次读取 1 MiB, 无可用插件时根列表 `available: false` 并附监视器状态; 令牌 `app.query`, 默认授权 61 个方法. 插件 `ResourceUri.Kind.DOCS` / `ResourceCatalog`: `resources/list` 仅在宿主回 `available: true` 时列出 `autojs6://docs/` 索引与 HTML 页面 (样式 / 脚本 / 图片经索引与模板可达), `resources/templates/list` 先以 `maxEntries 1` 探测再决定是否列出 docs 模板, `_meta.docsCatalogStatus` 携带宿主状态或失败码 (无此方法的旧宿主为 CAPABILITY_DENIED). JVM: 宿主 NodeBridgeDocsCatalogTest 7/7 与 MCP 包 32/32, 插件 235/235. DEVICE: AVD_API_33 (x86_64, 正式签名宿主 5280 + 插件 49 + Offline-Docs 42) 列出 333 资源 / 4 页 (docs 130 = 索引 + 129 页, 均为 HTML), 索引 fileCount 185 / totalBytes 10615957 / contentVersion 6.8.0, index.html 23425 字节, app.html 68638 字节, PNG blob 46200 字节, 单次读取 27-40 ms; 卸载文档插件后 1 s 内 docs 消失 (status not_installed, 模板 2 项, 读取回 INVALID_ARGUMENTS), 重装后 1.8 s 内恢复 130 项. 详见 docs/dev/p35-resources-prompts.md)
...
- [x] (宿主) `test-apps/mcp-server-conformance` (可选, 与 AI / DEX 家族同形): 确定性假插件 + 恶意 Binder 用例 (错误契约版本, 超限 Bundle, 重复回调, 死亡回调), 加入宿主 `pluginConformanceTestApps`. (SOURCE: 宿主仓库分支 `mcp-p6-conformance-app` 提交 f13a85ef8, 已通过 merge commit aa35b47aa 合入 master: `test-apps/mcp-server-conformance` 五个场景组件 normal / wrong-contract-version / oversized-bundle / duplicate-callback / death-callback 加 INFO 服务, JVM 用例 17 项, 已加入 `pluginConformanceTestApps` 与 CI; 宿主 `McpServerAndroidConformanceTest` 6 项 opt-in (`-e autojs.mcpServer.conformance.enabled true`), AVD_API_33 x86_64 上以 debug 签名宿主 5280 全部通过. 因 `McpServerPluginHost` 钉死正式包名, 测试直接按组件绑定假插件, 覆盖 validatePluginInfo / McpServerBundles 上限 / 真实 McpHostCapabilityBroker, 不覆盖 McpServerSessionController 重绑.)
...
目标: 首个公开版本 v1.0.1 发布并进入官方插件索引; 宿主随版本发布契约与开关.
...
- [x] (文档) 插件中心说明 (`@raw/plugin_instruction`, 10 语言) 与 `plugin_description`. (SOURCE: `.readme/template_plugin_instruction.md` 经 `generate_markdown.py` 生成 `res/raw{,-en,-zh,-zh-rHK,-zh-rTW,-ja,-ko,-ru,-ar,-es,-fr}/plugin_instruction.md` 共 11 份 (简介 / 版本状态 / 使用方法 5 步 / README 与 ROADMAP 链接), 状态段随 README 同步为 `版本 1.0.1` 并写明宿主要求; `plugin_description` 在 `values` 与 9 个语言限定 `values-*` 的 strings.xml 中各一条, `McpServerPluginInfo` 以 `R.string.plugin_description` / `R.raw.plugin_instruction` 交给宿主插件中心. 本条无代码改动, 复核后勾选.)
- [x] (插件) `.changelog` 10 语言 `v1.0.0` 条目 (`feature` / `dependency`), 生成 `assets/doc/CHANGELOG-*.md`; `VERSION_NAME=1.0.0` 与文件名断言一致. (SOURCE: `.changelog/lang_*.json` 10 语言的 v1.0.0 含 hint 1 / feature 22 / dependency 4 / improvement 1 / fix 6, v1.0.1 (维护者于 2026-09-15 起用的当前 VERSION_NAME) 含 feature 1 / improvement 4; `generate_markdown.py` 生成 `assets/doc/CHANGELOG-*.md` 14 份, 由 `--check` 与 CI 的 markdown 工作流守护; `appendDigestToReleasedFiles` 先断言发布 APK 文件名等于 `autojs6-plugin-mcp-server-v<VERSION_NAME>.apk` 再追加 CRC32, 忽略的 `releases/` 已有 v1.0.0-e9042e6d 与 v1.0.1-04135447 两份产物. 注: 当前 VERSION_NAME 为 1.0.1, 首个公开发布的 tag 应与之一致 (v1.0.1) 而非本条原文的 1.0.0; 两个版本的 released_date 均为 2026/09/15, 发布时由维护者改为实际日期.)
- [x] (宿主) 宿主 changelog 10 语言 (抽屉开关 `feature`), 宿主 `docs/dev/mcp-server-protocol-v1.md` 定稿; 若涉及脚本公开 API (本 Roadmap 默认不涉及) 才同步文档 / d.ts / Ace 仓库. (SOURCE: 宿主 `.changelog/lang_*.json` v6.8.0 已含抽屉开关 feature ("MCP Server drawer switch with installation, activation, enablement, authorization, and compatibility guidance...") 与后续 MCP 相关 feature / fix / improvement 条目 (含本日 `app.listDocs` / `app.readDoc` 一条, 分支 `mcp-p35-docs`); `docs/dev/mcp-server-protocol-v1.md` 首段标注 `Status: versioned V1 contract`, 含 Binder 面, Bundle 键, 状态机, 授权与上限, P1.3 / P3.5 桥方法, 生命周期, 描述符约定, P4 设置扩展与一致性测试应用各节, 无 TODO / 草案标记, 视为定稿; 本路线图不涉及脚本公开 API, 无需同步 d.ts / Ace 仓库. 本条无插件代码改动, 复核后勾选.)
- [x] (发布) 验证顺序: `py .python/generate_markdown.py --check`, `:app:testDebugUnitTest`, `:app:assembleDebug :app:assembleDebugAndroidTest`, `:app:lintDebug`, instrumentation (AVD), `:app:appendDigestToReleasedFiles` (签名齐全, 单 APK `autojs6-plugin-mcp-server-v1.0.0-<CRC32>.apk`). (ANDROID_BUILD + BINDER: 2026-09-16 在构建 54 的工作树按此顺序执行: `generate_markdown.py --check` MARKDOWN_OK 10 语言 36 产物; `:app:testDebugUnitTest` 235 / 235; `:app:assembleDebug` 得 `autojs6-plugin-mcp-server-v1.0.1.apk`, `:app:assembleDebugAndroidTest` 得 `app-debug-androidTest.apk`; `:app:lintDebug` 0 错误 / 8 警告 (依赖版本与图标重复类, 与 P6 一致); AVD_API_33 (`emulator-5554`, x86_64) instrumentation `OK (35 tests)`, 34 通过 + 1 假设跳过 (`killListenerProcess` 需 `-e mcpKillListener true`), 109.9 s; `:app:appendDigestToReleasedFiles` 经 `assembleRelease` 断言文件名后产出 `releases/autojs6-plugin-mcp-server-v1.0.1-e6f720e3.apk` (1377446 字节, versionCode 54, apksigner 签名者 CN=SM003). 期间修正一处测试节奏问题: `McpServerHostSessionTest.workspaceToolsBridgeArgumentsPayloadsAndSeparateDangerousSwitches` 在预热后的 x86_64 模拟器上 1 s 内发出超过 20 个请求, 被监听器自身的 20 请求 / s 上限以 429 拒绝 (构建 53 全量运行 2 / 2 复现, 单独运行通过, 真机较慢未触发), 现按 `Retry-After` 重试一次, 产品行为未改. 注: 文件名中的版本为 1.0.1 (当前 VERSION_NAME) 而非本条原文的 1.0.0; CRC32 随 versionCode 变化, 维护者须在打 tag 的提交上重新执行 digest 任务并以该产物发布.)
- [x] (发布) GitHub Release (首个公开 tag `v1.0.1`, 签名 APK, changelog 文本); `AutoJs6-Official-Plugins-Index` 经 `tools/generate_official_plugin_index.py` 重新生成 (不手改 JSON), 追加 `release-manifests/<packageName>/<versionCode>.json` (真实 sha256 / size / signer); 宿主插件中心可发现并安装, 两台设备完成 P2 客户端验收. (RELEASE + INDEX + DEVICE, 2026-09-16: 仓库与 [Release v1.0.1](https://github.com/SuperMonster003/AutoJs6-Plugin-MCP-Server/releases/tag/v1.0.1) 已公开. Tag 指向 `d2731e8ffed1efc2a222b57cc782f4b65e29adec`, versionCode 57; 全部语言 v1.0.1 与此前未公开的 v1.0.0 released_date 已更新为 `2026/09/16` 并重新生成文档. 在最终 tag 提交重跑 digest, 以该 APK 和 `--commit d2731e8ffed1efc2a222b57cc782f4b65e29adec` 生成清单; 公开资产名 `autojs6-plugin-mcp-server-v1.0.1-3b9fc1b0.apk` 与 artifacts.name 完全相同, 1377442 字节, SHA-256 `c8b5d521358fb196b746fc417797f5d283b15dcb287ebc0f5a71c22fbcd565b5`, 签名 SHA-256 `31a681fcfffb3e428420cae280ded89292b12a3b0f59e19b7a73e32a8ae4c213`; 下载公开资产后复核一致. 索引仓库 `release-manifests/io.github.supermonster003.autojs6.plugin.mcp.server/57.json` 已纳入 `0084f13` 并推送 main, 全量生成 37 个插件条目, 单元测试 33/33, MCP 恰有一个条目且资产 / 版本 / 源码提交 / 签名均匹配. Sony XQ-DQ72 API 33 与 Xiaomi 23046RP50C API 35 已通过插件中心更新安装到 1.0.1 / 57, 拉取设备 base APK 的 SHA-256 均与公开资产相同. Inspector 与生命周期证据见 `docs/dev/p7-public-release.md`; Claude Code 2.1.270 后续通过维护者指定的 ACP 配置 `Claude via AIGoCode [Fabel 5.1 (C)]` (model `claude-fable-5-1[1m]`) 在两台公开 APK 上完成真实 device_info / script_run. 两台均先收到 PAIRING_REQUIRED, 经手机允许后成功; Sony 5 轮 / 15089 ms, Xiaomi 确认配对后的成功会话 3 轮 / 10393 ms, script_run 均 finished 且控制台含验证标记. 临时客户端配对 / CLI 配置 / adb forward 已清理, 开发者模式与停止状态已恢复. Sony 本轮开始时已升级为开发版 1.0.2 / 60, 为测试临时无清数据回装公开版, 完成后恢复原开发 APK 并核对哈希; 平板保持公开版. 两台完整 P2 客户端 gate 已满足, 本条据此勾选. 第 8 项与真机 docs 证据按本次要求留给下次会话.)
- [x] (测试) ColorOS 或同类设备的安装后激活实测 (插件中心出现 "激活", 点击后可用, 重启后仍可发现); 无设备时如实记录 "未执行". (DEVICE: ColorOS 未执行, 手头无 ColorOS 设备 (在线设备为 Sony XQ-DQ72 API 33, Xiaomi Pad HyperOS API 33 与 AVD). 同类 OEM 的安装后激活路径已在 P4 覆盖 (2026-09-15 记录: 未安装 / 需要激活 / 已停用 / 版本不兼容四态引导与恢复在 API 28 / 31 / 33 / 33 / 35 真机各通过一轮, 含 Xiaomi HyperOS; 新装后抽屉需宿主重启才刷新的观察仍是宿主待办), P6 生命周期矩阵覆盖宿主 / 插件进程被杀后的重新发现. 整机重启后的可发现性未单独实测 (Xiaomi Pad 重启后需图案解锁, 不由 adb 驱动). 取得 ColorOS 设备后按本条原文补做并回填.)
...
| `autojs6://docs/{path}` (模板, 可选) | 离线文档 (需 Offline-Docs 插件, 未安装时不列出; 目录索引 `autojs6://docs/`) | `app.listDocs` / `app.readDoc` (D20, 宿主分支 `mcp-p35-docs`) |
...
### 2026-09-16: 合并宿主 P6 一致性测试分支并清理 worktree

- 完成: 宿主 `master` 通过 merge commit `aa35b47aa` 合入 `mcp-p6-conformance-app` (`f13a85ef8`), 双亲为原 master `5d4a23507` 与一致性测试分支. 合并无冲突, 共 20 个文件, 保留两边提交历史; 合并后的 `app/src/main` 与原 master 完全一致. `git merge-base --is-ancestor mcp-p6-conformance-app master` 通过, 本地源分支保留, 未推送远端.
- 验证: worktree 临时切换到合并提交 `aa35b47aa` 后执行测试应用 JVM 17/17, 宿主 MCP 包与插件中心 MCP 注册 JVM 21/21, 均无失败或跳过; 测试应用 `assembleDebug` / `lintDebug` 与宿主 `assembleAppDebug` / `assembleAppDebugAndroidTest` 通过, Gradle 9.7.1 / JDK 21, 宿主 6.8.0 / 5280, 构建 1m 52s. 测试应用 lint 0 errors / 2 warnings (DataExtractionRules, MissingApplicationIcon). AVD_API_33 x86_64 临时只读实例 `emulator-5610` 安装同一 debug 签名的合并树 APK 后, `McpServerAndroidConformanceTest` 6/6, 8.892 s; 测试结束关闭该实例. 原始输出保留于忽略的 `build/p6-conformance-merge-gradle.log`, `build/p6-conformance-merge-instrumentation.log` 与 `build/p6-conformance-merge-evidence/`.
- 清理: 已确认 worktree 无未提交文件, 忽略内容仅为构建缓存与产物, 全部提交已合入 master. Git worktree 登记已移除, 原 `AutoJs6-mcp-p6-conformance` 目录已不存在. Git 删除目录遇到 Windows 长路径限制, 后续两次 PowerShell 删除被自动审批以 `blocked by policy` 拒绝; 改为可逆归档, 残留保留在本仓库忽略的 `build/worktree-cleanup/AutoJs6-mcp-p6-conformance` 与 `build/worktree-cleanup/AutoJs6-mcp-p6-conformance-remaining`. 构建 JAR 的文件占用在停止已空闲的 Gradle 9.7.1 守护进程后释放. 这两份残留归档未永久删除.
- 范围: 宿主仅合并已有测试, 构建注册与开发文档, 公共契约和运行时依赖无变化; 未重跑宿主全量 JVM / lint / release 与完整服务器生命周期矩阵. 本仓库仅更新路线图与提交计数, 文档生成检查和路线图标点检查通过, 不重复插件 APK 构建与设备测试.
- 下次会话建议起点: P7 文档与发布 gate; 宿主分支已合并, worktree 原路径和登记均已清理, 残留归档可由维护者按需删除.

### 2026-09-16: P3.5 可选离线文档资源

- 完成 (宿主仓库, worktree `D:/idea-projects/AutoJs6-mcp-p35-docs`, 分支 `mcp-p35-docs`, 提交 04c9c16b7, 基于 master aa35b47aa, 未合并, 由维护者合并): 只读 `app.listDocs(path?, options?)` / `app.readDoc(path, options?)` 并入 `NodeBridgeSamplesBridge` 的 JSON 层, 新增 Android-free 的 `NodeBridgeDocsCatalog`, 源为 `OfflineDocsPluginStateMonitor.loadedPlugin` (监视器仍在 LOADING 时同步 `OfflineDocsPluginHost.load` 兜底), 只允许读取已校验清单中的路径, 目录由清单推导, 路径规则与示例目录一致; 根列表附 `status` / `packageName` / `contentVersion` / `contractVersion` / `entryPoint` / `fileCount` / `totalBytes`, 无可用插件时 `available: false` 加监视器状态与 `errorCode`. 注册: `NodeBridgeModules.appMethods`, 权限清单 `app.query`, `McpCapabilityGrant.DEFAULT_METHODS` (61 个方法), 一致性 instrumentation 的稳定方法集, `docs/dev/mcp-server-protocol-v1.md` 三处, 宿主 changelog v6.8.0 improvement 10 语言并重新生成 README. JVM: NodeBridgeDocsCatalogTest 7/7, MCP 包与桥权限清单 32/32.
- 完成 (本仓库, 构建 49): `ResourceUri.Kind.DOCS` (`autojs6://docs/`, 模板 `autojs6://docs/{+path}`), `ResourceCatalog.list` 在宿主 `available: true` 时列出 docs 索引与 HTML 页面 (样式 / 脚本 / 图片 / 字体经目录索引与模板可达), 否则不列出并以 `_meta.docsCatalogStatus` 记录宿主状态 (not_installed / disabled / ineligible / loading) 或失败码 (无此方法的宿主为 CAPABILITY_DENIED); `templates()` 改为 suspend, 以 `maxEntries 1` 探测后决定是否列出 docs 模板; 目录索引与页面 / blob 读取复用示例路径. `write_autojs6_script` 中英提示各加一句 "若列出 autojs6://docs/ 则阅读所依赖模块页面". JVM: ResourceCatalogTest 15/15 (新增 2 项), 全量 235/235; 文档生成检查 MARKDOWN_OK; README 功能句与 changelog v1.0.1 feature 各 10 语言.
- 证据: AVD_API_33 (x86_64, `emulator-5554`) 安装 worktree 构建的正式签名宿主 5280 (worktree 复制了忽略的 `sign.properties` 与 `app/sm003.jks`, 与 ROADMAP 第 219 行插件仓库的做法一致), 插件 49, Offline-Docs 插件 42 (内容版本 6.8.0, 185 个已校验文件 / 10.6 MB). `p35_docs.py`: 宿主授权 61 个方法; 模板 3 项 (41 ms); `resources/list` 333 资源 / 4 页 (示例 201, docs 130 = 索引 + 129 个 HTML 页面, 992 ms 含 4 页); 根索引 132 项 (38 ms); index.html 23425 字节 (27 ms), app.html 68638 字节 (40 ms), `images/...-dark.png` blob 46200 字节 (29 ms), 均未截断; 错误探针: 缺页 / 目录当文件 / 文件带尾斜杠均 INVALID_ARGUMENTS (-32602, 带宿主消息), `..` 段为 Invalid AutoJs6 resource URI. 卸载文档插件后 1 s 内 `docsCatalogStatus = not_installed`, 列表 203 项无 docs, 模板 2 项, 读取 index.html 回 INVALID_ARGUMENTS (offline documentation is not available); 重装后 1.8 s 恢复 130 项与 3 个模板. 原始记录在忽略的 `build/p35/`.
- 备注: 新装宿主首次启动弹 "All files access" 对话框挡住抽屉, 以 appops 授予 MANAGE_EXTERNAL_STORAGE 并关闭一次即可; `read_token` 依赖插件 androidTest APK. 真机 (Sony / 小米) 仍运行不含这两个方法的正式宿主 5280, 插件在其上不列出 docs 且 `docsCatalogStatus = CAPABILITY_DENIED`, 真机 docs 证据待宿主分支合并并发布后补做.
- 下次会话建议起点: P7 文档与发布 gate (README 十语言复核, 插件中心说明, changelog 与版本名断言, 宿主 changelog / 协议文档定稿, 验证顺序, GitHub Release 与官方索引须维护者确认); 维护者侧: 合并 `mcp-p35-docs`, 清理 worktree `AutoJs6-mcp-p35-docs` (含复制的签名文件).

### 2026-09-16: P7 文档与发布 gate

- 完成 (本仓库, 构建 50 ~ 56): 第 1 项 README 十语言 (状态段改为 `版本 {{ version_name }}` 并写明宿主要求与 docs 前提, `功能` 节措辞, 新增由 `tool-catalog.snapshot.json` 生成的 37 行工具清单表, 真实截图 `docs/images/readme/{drawer,settings}-{en,zh}.png`, 去掉未交付的公网隧道 / OAuth 表述); 第 2 ~ 4 项复核后勾选 (插件中心说明 11 份与描述 10 份, changelog v1.0.0 / v1.0.1 与文件名断言, 宿主 changelog 与协议文档定稿); 第 5 项验证链在构建 54 全部通过 (MARKDOWN_OK, JVM 235 / 235, assembleDebug / AndroidTest, lint 0 错误 8 警告, AVD instrumentation 35 项, digest `v1.0.1-e6f720e3`), 修正一处测试节奏问题 (预热模拟器触发 20 请求 / s 上限, 测试改为按 Retry-After 重试一次); 第 6 项发布材料与准入清单草案就绪并经索引生成器离线绑定校验, 发布动作留给维护者, 保持未勾选; 第 7 项如实记录 ColorOS 未执行.
- 未完成: 第 6 项的 GitHub 仓库 / Release v1.0.1 / 索引清单提交 / 插件中心安装验证 (维护者, 步骤已写在条目内); 第 8 项发布后回填 ("当前结论" 表, 附录 D 证据, 维护者记忆, 宿主 a11y Roadmap 的跨运行时消费方列表). 真机 docs 资源证据待含 `app.listDocs` / `app.readDoc` 的宿主发布后补做.
- 工具 (忽略目录 build/tools): `p7_shots.py` (抽屉 / 设置页截图, 复用 p4_ui / p4_states), `patch_p7_*.py` (README / 条目 / 测试补丁), `p7_release_manifest.py` (aapt2 + apksigner + sha256 生成准入清单草案到 build/p7), `p7_manifest_dryrun.py` (以草案喂索引生成器 `bind_release_artifacts()` 做离线校验).
- 下次会话建议起点: 维护者侧先合并宿主 `mcp-p35-docs` 并清理 worktree `AutoJs6-mcp-p35-docs` (含复制的签名文件), 在打 tag 的提交上重跑 digest 并发布 v1.0.1, 按第 6 项步骤提交索引清单; 之后本仓库执行 P7 第 8 项回填与真机 docs 证据; P8 (公网隧道与 OAuth 2.1) 为可选, 进入前重新评估需求.

### 2026-09-16: 首个公开 Release v1.0.1 与官方索引安装

- 宿主: `master` 通过 `d4e05a81d` 合入 `mcp-p35-docs` (`04c9c16b7`), 保留分支历史与宿主无关工作区内容. 合并树的 docs / samples / bridge 权限 / MCP 包 JVM 32/32, `assembleAppDebug` 与 `assembleAppDebugAndroidTest` 通过. `AutoJs6-mcp-p35-docs` worktree 登记和目录已移除, 其中复制的忽略签名文件一并清理; 源分支保留, 宿主合并尚未推送.
- 发布: 按维护者要求创建公开 GitHub 仓库, 首个公开 tag 为 v1.0.1 / 57 (`d2731e8`), 不是历史规划的 1.0.0. Changelog 十语言实际日期为 2026/09/16. 最终 tag 上重跑 `appendDigestToReleasedFiles`, 以明确 APK 和 `--commit` 生成准入清单; 上传唯一资产 `autojs6-plugin-mcp-server-v1.0.1-3b9fc1b0.apk`, 1377442 字节. Release / 清单 / 下载文件 / 两台设备安装文件的哈希一致, 完整收据与验证链接见 `docs/dev/p7-public-release.md`.
- 索引: `0084f13` 已推送 main, 清单位于包名目录的 `57.json`, 全量生成器生成 37 个条目, 单元测试 33/33. 首次运行遇到已发布 ImGui Manifest 的宿主版本占位符, 补充有限的静态 Gradle 赋值与同 tag 源码常量解析, 对未知表达式和路径保持拒绝, 加 5 项测试; 未手改生成的索引 JSON.
- 验证: 发布源 JVM 235/235, debug / androidTest / release 构建通过, lint 0 错误 / 8 警告; API 33 私有只读模拟器 instrumentation `OK (35 tests)` (34 通过 + 1 可选 killListenerProcess 假设跳过), 98.192 s. 空闲停止测试先前会把短暂的 socket 超时当作停止, 已在独立提交 `5e67a73` 中同时要求持久化状态为 stopped; 正式 release 不含该测试代码. Tag 的 Build integrity 与 Markdown integrity 均通过, 含 API 24 / 35 CI. 临时模拟器已关闭.
...
### 2026-09-16: 补齐公开版的 Claude Code 客户端验收

- 完成: 维护者提供 ACP 的 `Claude via AIGoCode [Fabel 5.1 (C)]` 配置, 将该配置的环境仅注入隔离的 Claude Code 2.1.270 子进程, model `claude-fable-5-1[1m]`. 两台设备以 Streamable HTTP + Bearer + USB 转发接入; 模型仅获准调用 device_info 与 script_run, 内置工具关闭. 两台均通过真实 tool_use / tool_result 事件核对设备型号与脚本 finished / 控制台标记, P7 第 6 项的缺口已补齐并勾选.
- 证据: Sony XQ-DQ72 API 33, 首次配对门拒绝两次, 手机允许后同一会话完成两工具, 5 轮 / 15089 ms; Xiaomi 23046RP50C API 35, 首会话在确认前用完重试次数, 通知允许后第二会话完成两工具, 3 轮 / 10393 ms. 测试前两台 APK 均核对为公开版 1.0.1 / 57 的 SHA-256 c8b5d521...565b5. Sony 已被更新为开发版 1.0.2 / 60, 本轮先保存开发 APK, 无清数据临时回装公开 APK, 验收后恢复原开发版并确认哈希 489560d9...57c63; 平板保持公开版. 详情见 `docs/dev/p7-public-release.md`.
- 清理: 两台本次 Claude 配对已撤销, 临时 CLI 配置目录与 adb forward 已移除, 开发者模式和停止状态恢复; ACP 文件哈希未变, 凭据未写入证据文件. 本轮验证 CLI 复用 ACP 网关配置, 未单独验证 JetBrains ACP 对话界面.
- 范围: 只更新发布证据, 路线图勾选与提交计数, 不改运行时实现, 公共契约, 依赖或已发布 APK. 主工作区另有正在进行的本地网络权限改动, 本轮证据在隔离的 `docs/p7-claude-release-validation` 分支提交, 未混入并行改动. 文档生成检查与文本标点检查通过; 未重复完整 APK 构建和 instrumentation, 因产品代码未变且本轮直接验证已发布 APK.
- 下次会话建议起点: P7 第 8 项回填 (当前结论 / 附录 D / 维护者记忆 / 宿主 a11y Roadmap) 与含 app.listDocs / app.readDoc 宿主的真机 docs 资源证据, 按原约定留待下次会话. 本轮不提前修改这些内容.
