...
- 工具目录与宿主桥接 (P2.3 起): 工具只在 `tools/ToolCatalog` 登记 (snake_case `<group>_<action>` (P3.4 明定的 `toast` 除外), 封闭 JSON Schema `additionalProperties: false`, 所属 D6 分组, 宿主方法与权限, 超时), `ToolCatalogTest` 的快照 `app/src/test/resources/tool-catalog.snapshot.json` 变更时 MUST 一并更新并写入更新日志; 分组开关经 `ToolPermissionStore` (`tool_groups.json`) 持久化, `ToolRegistry` 只向 SDK 注册已启用的工具并在 `tools/list` / `tools/call` 前刷新, 关闭的工具由 `ToolGate` 以 `isError` + `TOOL_DISABLED` 回答; 宿主调用一律经 `HostBridgeClient` (单调 `mcp-N` id, 超时 = 请求超时 + 2 s 宽限, 并发上限取宿主 grant 且 <= 4, 错误类别按附录 A.5 映射), MUST NOT 绕过它直接调用 broker; 宿主死亡时监听器继续运行, 宿主工具回 `HOST_UNAVAILABLE`, 新的 `openServer` 替换已死会话, 宿主仍存活时回 `already_open`; `McpServerPluginService.openServer` MUST 经 `HostCallerVerifier` 校验调用方 (宿主 UID + 版本 + 同签名), `getInfo` / `getCapabilities` 只受清单权限保护; 日志与 `tool_call` 事件只含工具名与客户端名, MUST NOT 记录脚本源码, 参数或结果. 宿主契约 AAR (`libs/mcp-server-api.aar`) 与 `common-plugin-api.aar` MUST 来自同一宿主构建并一起换锁.
...
- 工作目录与设备工具 (P3.4 起): `WorkspacePath` 在插件侧拒绝路径逃逸, 宿主继续校验 canonical 路径. 文件读写最大 1 MiB, 写入还必须在 Binder dispatch 前按完整 JSON 的 UTF-8 字节数检查宿主请求预算 (当前 96 KiB). 大 JSON 结果使用仅含 `payload` 的结果包装与 `application/json` PFD, 由 `HostBridgeClient` 解包并关闭, 上限仍为契约 8 MiB. `editor_open` 的行列号从 1 开始, 宿主转换为编辑器从 0 开始的位置. `app_list` 使用 `package_manager.listApps`, 不改变脚本依赖包的 `list`. `allowShellRoot` 单独持久化且默认 false; `shell_exec(root=true)` 必须同时满足 shell 组开关, allowShellRoot 与宿主 shell.root grant, 每次执行重新读取开关. stdout 与 stderr 合计最大 256 KiB, 宿主与插件都按 UTF-8 字节截断. `device_ensure_accessibility` 调用宿主既有自动启用控制器, 不放宽 Android 权限或自动打开设置页.
...
- `McpServerHostSessionTest` (P2.3 起) MUST 覆盖: 假 broker 下 `openServer` -> `running` (端点, 端口, `hostAvailable`, 契约版本) -> `tools/list` 全目录 -> 配对 -> `device_info` 与 `script_run` 的桥接信封 (module / method / args / timeoutMs / permissions / 客户端名) -> 宿主失败映射 -> 参数校验 -> 分组关闭后 `tools/list` 隐藏且 `tools/call` 回 `TOOL_DISABLED` -> `stop` / `close`; 以及 `already_open`, `invalid_config`, 无宿主会话时 `HOST_UNAVAILABLE`. `McpServerPluginContractTest` 还 MUST 覆盖真实 `IMcpServerPlugin` 的 `getInfo` / `getCapabilities` 能力键与非宿主调用 `openServer` 得到 `SecurityException`. 宿主仓库的 `McpServerPluginRoundTripTest` 是 P1 验收的 Binder 往返证据, 宿主契约变更时 MUST 重跑; 脚本分组 (P3.1 起): `script_run` 抛错 -> `status = error` + `exception.line`, `script_run_file` 不等待 -> `running` + `hint`, `script_list` / `console_tail` (`lines` / `level` 透传, 结果去掉 `schema`) / `script_stop` (id) / `script_stop_all` (`{scope: "host"}`) 的信封与结果整形; 源以 `slow` 开头的 `script_run` (假宿主 2.6 s 后应答) 带 `progressToken` 时 MUST 在同一 `tools/call` 响应流中收到带最新控制台行的 `notifications/progress`.
